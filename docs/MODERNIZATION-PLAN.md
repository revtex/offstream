...
Delivered in four PRs. **PR 1 (shell scaffold, DI, navigation, design tokens), PR 2 (Record page) and PR 3 (Settings and Advanced pages) have landed;** PR 4 is the FlaUI sweep and polish.
...
**Exit:** FlaUI suite covers every control; key-parity test passes; side-by-side behavioural review against the reference app (behaviour compared, chrome and wording deliberately not copied). **Key-parity met** in PR 1; PR 3 brings the total to 704 tests green, 131 of them in `Offstream.UI.Tests`.
